...
async function fetchClient(email: string, userInputPassword: string) {
  ...
  if (userExistsData.length > 0) {
    await db.query("CALL Logowanie(?, ?, TRUE)", [
      userExistsData[0]?.["first_name"],
      email,
    ]);
  } else {
  ...
  const roleResult = await db.query(queryGetRole, [userExistsData[0]?.["id"]]);
  const userRole = helper.emptyOrRows(roleResult);
  let role = "";
  ...
  return { response: { statusCode: 200 }, email, userRole: role };
}
