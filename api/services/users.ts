...
async function get(
	...
	// Build query
	let query =
		"SELECT u.id, u.email, u.first_name, u.last_name, u.phone_number, u.address, u.gender, u.birth_date,u.create_time, u.img, e.role FROM Employees e RIGHT JOIN Users u ON e.Users_id=u.id";
	let queryParams = [];
...
async function update(id: number, user: User) {
	...
	const checkResult = await db.query("SELECT '' FROM Users WHERE id = ?", [
		id,
	]);
...
async function updateRole(id: number, role: string) {
	...
	try {
		...
		const checkResult = await db.queryTransaction(
			connection,
			"SELECT role FROM Users u LEFT JOIN Employees e ON e.Users_id=u.id WHERE u.id = ?;",
			[id]
		);
		const checkRows = helper.emptyOrRows(checkResult);
		...
		if (checkRows[0].role === null) {
			...
			if (role !== "client") {
				const result = await db.queryTransaction(
					connection,
					"INSERT INTO Employees (Users_id, role) VALUES (?, ?)",
					[id, role]
				);
		...
		if (role === "client") {
			const result = await db.queryTransaction(
				connection,
				"DELETE FROM Employees WHERE Users_id = ?",
				[id]
			);
			if ((result as ResultSetHeader).affectedRows === 0) {
				throw new Err("Could not update role");
		...
		let result = await db.queryTransaction(
			connection,
			"UPDATE Employees SET role = ? WHERE Users_id = ?",
			[role, id]
		);
...
async function addImg(id: number, path: string) {
	...
	const result = await db.query("UPDATE Users SET img = ? WHERE id = ?", [
		path,
		id,
	]);
	const affectedRows = (result as ResultSetHeader).affectedRows;
