...
const CheckoutForm: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:6868/stripe/create-payment-intent",
        {
          method: "POST",
  ...
  const saveRentalCar = async () => {
    const rentalDates = JSON.parse(
      sessionStorage.getItem("rentalDates") || "{}"
    );
  ...
  const saveReservation = async () => {
    const reservationDates = JSON.parse(
      sessionStorage.getItem("reservationDates") || "{}"
    );
  ...
  const sendConfirmationEmail = async () => {
    try {
      ...
      if (response.status === 200) {
        console.log(
          "E-mail potwierdzający został wysłany na podany adres e-mail."
        );
      } else {
