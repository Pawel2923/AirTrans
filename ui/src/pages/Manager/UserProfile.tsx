...
const UserProfile = () => {
	...
	useEffect(() => {
		if (userInfo) {
			...
			if (userInfo[0].img) {
				if (imgRef.current) {
					imgRef.current.src = filesService.getImgUrl(
						userInfo[0].img
					);
				}
			}
	...
	return (
		<>
			...
			<div className="container-flid manager-block mt-3 p-4">
				<div className="row">
					<div
						className={`col-lg-6 ${classes["user-info-container"]}`}
					>
						<h2>Twoje dane</h2>
