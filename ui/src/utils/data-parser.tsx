...
export const ticketsDataParser = (tickets: Tickets) => {
	...
	ticketData.departureDate = tickets.departure.split("T")[0].split("-").reverse().join(".");
	...
	ticketData.updateTime = tickets.update_time.slice(0, 19).replace("T", " ").split(" ").map((el, index) => index === 0 ? el.split("-").reverse().join(".") : el).join(" ");
